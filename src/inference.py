...
def sliding_window_embed(
    ...
    roi_size: Sequence[int] | int,
    sw_batch_size: int,
    embedder: Callable[..., torch.Tensor],
    overlap: Sequence[float] | float = 0.25,
    mode: BlendMode | str = BlendMode.CONSTANT,
    sigma_scale: Sequence[float] | float = 0.125,
    padding_mode: PytorchPadMode | str = PytorchPadMode.CONSTANT,
    ...
    **kwargs: Any,
    ...
    """
    Sliding window inference for generating embeddings over a volume with proper handling of overlapped regions.

    Args:
        inputs: input tensor to be processed (assuming NCHW[D])
        roi_size: spatial window size for inference
        sw_batch_size: batch size for running windows
        embedder: embedding function taking input tensor NCHW[D] -> N,L,D where L is number of patches
                 and D is embedding dimension
        overlap: overlap between windows (default 0.25)
        mode: blending mode for overlapped regions - "constant" or "gaussian" (default "constant")
        sigma_scale: standard deviation for gaussian blending (default 0.125)
        padding_mode: padding mode for inputs when roi_size > inputs (default "constant")
        cval: constant value for padding (default 0.0)
        sw_device: device for window computation (default: same as inputs)
        device: device for output tensor (default: same as inputs)
        progress: show progress bar
        args: additional args for embedder
        kwargs: additional keyword args for embedder

    Returns:
        torch.Tensor: Blended embeddings for the full volume
    """
    compute_dtype = inputs.dtype
    num_spatial_dims = len(inputs.shape) - 2
    batch_size, channels, *image_size_ = inputs.shape
    ...
    # Calculate scan intervals and slices
    overlap = ensure_tuple_rep(overlap, num_spatial_dims)
    scan_interval = _get_scan_interval(image_size, roi_size, num_spatial_dims, overlap)
    ...
    importance_map = compute_importance_map(
        valid_patch_size, mode=mode, sigma_scale=sigma_scale, device=sw_device, dtype=compute_dtype
    )
    ...
    first_embed = embedder(inputs[tuple(slice(0, 1) for _ in range(len(inputs.shape)))].to(sw_device))
    embed_dim = first_embed.shape[-1]
    output_shape = [batch_size, num_win, embed_dim]

    output_embeddings = torch.zeros(output_shape, dtype=compute_dtype, device=device)
    count_map = torch.zeros([batch_size, num_win, 1], dtype=compute_dtype, device=device)
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        embeddings = embedder(window_data, *args, **kwargs)

        # Apply importance map
        embeddings = embeddings * importance_map.view(-1, 1)

        # Add to output and count map
        for idx, embedding in zip(slice_range, embeddings):
            batch_idx = idx // num_win
            win_idx = idx % num_win
            output_embeddings[batch_idx, win_idx] += embedding.to(device)
            count_map[batch_idx, win_idx] += importance_map.to(device)

    # Normalize by count map
    output_embeddings = output_embeddings / (count_map + 1e-8)

    # Remove padding if needed
    if any(pad_size):
        # Implement padding removal for embeddings if needed
        pass
